...
public:
  Option(double time, double vol, double rate, double underlyingprice,
         double strike, double dividend, double timetilldividend,
         OptionsType type, OptionsSide side, UnderLyingType underlyingtype);
  Option(double time, double vol, double rate, double underlyingprice,
         double strike, OptionsType type, OptionsSide side,
         UnderLyingType underlyingtype);
  Option(double time, double vol, double rate, double underlyingprice,
         double strike, double foreigninterest, OptionsType type,
         OptionsSide side, UnderLyingType underlyingtype);
  Option(double optionsprice, int time, double rate, double underlyingprice,
         double strike, OptionsType type, OptionsSide side,
         UnderLyingType underlyingtype);

  Option(double time, double vol, double rate, double underlyingprice,
         double strike, double dividend, double timetilldividend,
         std::vector<double> timetillexcercises, OptionsType type,
         OptionsSide side, UnderLyingType underlyingtype);
  ...
  double getForeignInterestRate() const {
    return underlyingType_ == UnderLyingType::Forex
               ? foreignInterest_
               : throw std::logic_error(
                     std::format("cannot get foreign interest arte for options "
                                 "that is not forex"));
  }
  std::vector<double> getTimeTillExcercises() const {
    return timeTillExcercises_;
  }
  ...
  void setTimeTillExpiration(double timetillexpiration) {
    timeTillExpiration_ = timetillexpiration;
  }
  void setVolatility(double vol) { volatility_ = vol; }
  void setRiskFreeRate(double riskfreerate) { riskFreeRate_ = riskfreerate; }
  void setForeignInterestRate(double foreigninterest) {
    foreignInterest_ = foreigninterest;
  }
  void setUnderlyingPrice(double price) { underlyingPrice_ = price; }
  void setStrikePrice(double strike) { strikePrice_ = strike; }
  void setB(double b) { b_ = b; }
  void setTimeTillExcercises(std::vector<double> exc) {
    timeTillExcercises_ = exc;
  }
